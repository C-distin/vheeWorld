...
const focusAreas = ["Education", "Mentorship", "Mental Health", "Resource Provision", "Community Outreach"]
            ...
            <div className="flex items-center gap-3 pt-1">
              {socials.map(({ icon: Icon, href, label }) => (
                <Link
                  ...
                  href={href}
                  aria-label={label}
                  className="flex items-center justify-center w-9 h-9 rounded-full border border-white/10 text-white/40 hover:border-purple-500/50 hover:text-purple-400 transition duration-200">
                  <Icon size={15} />
                </Link>
              ))}
            </div>
            ...
            <ul className="space-y-3">
              {quickLinks.map(({ label, href }) => (
                <li key={label}>
                  <Link
                    href={href}
                    className="group flex items-center gap-2 text-sm text-white/45 hover:text-white transition duration-200">
                    <span className="block w-0 h-px bg-purple-400 group-hover:w-4 transition duration-300" />
                    {label}
                  </Link>
                </li>
              ))}
      ...
      <div className="relative border-t border-white/[0.06]">
        <div className="max-w-7xl mx-auto px-6 md:px-12 py-6 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-[11px] text-white/20 tracking-wide">
            © {currentYear} VheeWorld Foundation. All rights reserved.
          </p>
